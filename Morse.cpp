/*
   Morse.cpp - Simple CW Keyer Morse Code encoding and decoding 
...
// Sends a audio Morse Response to a Keyer Command input via paddles
{
  ...
  while (!transmission_done) {
    ...
    // last element of each character already has an inter element space so we use 2 X Dit not 3)
    }
...
void audio_send_morse_character(uint8_t send_char, uint32_t ditDuration_ms) 
{
  ...
  do {
    ...
    switch (send_state)
    {
      case START : {  // The initial state when sending a morse element
        ...
        tone(11, 600);  // Fix this should use proper defines for PIEZO PIN and Tone Frequency 
        break;
      }
      case SEND_DIT : {
        if (millis() > timeout_time) {
          noTone(11);       // turn off keyer sidetone PIEZO_SPKR_Pin
      ...
      case SEND_DAH : { // Ok so you probably figured out that this code is identical to the case SEND_DIT, but I think this improves readability
        if (millis() > timeout_time) {
          noTone(11);       // turn off keyer sidetone PIEZO_SPKR_Pin
          timeout_time = millis() + ditDuration_ms; // Inter-element time same as DIT duration
          send_state = SEND_INTER_ELEMENT;
      ...
      case SEND_SPACE : { // This isn't needed at all as SPACE character is handled as a special case
        break;
